#' Barrier Behaviour Analysis (BaBA)
#'
...
#' @examples
#'   \dontrun{
#'     ## Run BaBA on mule deer data using the default methods, exporting event
#'     ## images to visually check event classifications.
#'     data("muleDeer")
#'     results_deer <- BaBA(animal = muleDeer, barrier = fences, d = 90,
#'                      export_images = TRUE, img_suffix = "DEER")
#'                      
#'     ## Run BaBA on caribou data using the updated methods, exporting burst
#'     ## images.
#'     wah.out <- BaBA_caribou(caribou = TRUE, animal = wah_all,
#'                             barrier = rd_final,
#'                             d = c(20000, 20000, 5000, 20000, 20000),
#'                             interval = 8, tolerance = 0, units = 'hours',
#'                             round_fixes = TRUE, crs = 'EPSG:6393',
#'                             export_images = TRUE, img_path = 'C:/BaBA_runs'),
...
#'        \item{\code{class}} {Character indicator of the movement classification assigned to the given burst. Primary classes include "Normal_Movement", "Quick_Cross", "Back_n_forth", "Bounce", "Trace", and "Unknown". Additional temporary indicators include "Normal_lclNA", "Unknown_insufficient_n", "Normal_sd0", "Unknown_cross_bounce", and "Unknown_not_enough_seasonal_pts_outside_buffers". These tier to the previous classifications but provide additional information about why that categorization was reached, for diagnostic purposes.}
...
BaBA_caribou <-
  function(animal, barrier, d, interval = NULL, tolerance = 0, units = "hours",
    ...
    # Classification step 1: generate encounter and burst data --------------
    ...
    for(k in unique(cross_coords$burstID)){
      ...
      for(j in 1:(nrow(encounter_i)-1)){
        ...
        if(nrow(int_check) > 0){
          ...
          ## information to split encounters into bursts below.
      ...
      ## which is used to identify different bursts based on crossing events.
    ...
    encounter_complete <-
      ...
      dplyr::mutate(
        ...
        ## Sum barrier differences and cumulative crossing to get unique
        ## indicators (per burstID) of barrier/crossing events
    ...
    # Classification step 2: classify bursts ----------------------------------
    ...
    for(i in unique(encounter$burstID)) {
      ...
      if(burst_i$cross_ind[2] == 0 &
        ...
        ## If there's an intersection, check for points on different sides of
        ## the barrier to indicate a true crossing
        if(nrow(int_check) > 0){
      ...
      animal_season_exclude <-
        animal_i %>% 
        ## Subset down to the most common season in the burst
      ...
      ## lasting the duration of the burst (moving window of the size of the
      ## burst) or 1000 hrs, whichever is shorter.
      ...
      for(ii in unique(animal_season_exclude$continuousID)) {
        ...
        duration_ii <- difftime(animal_ii$date[nrow(animal_ii)], animal_ii$date[1], units = units)
        ## Calculate straightness only if at least as long as the burst duration
      ...
      ## Determine the confidence interval for "normal movement" as mean +/-
      ## sd_multiplier * sd
      ...
      ## Testing showed a need to prioritize trace behavior, otherwise it tends
      ## to get classified as normal mvmt when a response to the barrier is
      ## clear.
      ...
      if(lcl_NA_check$lcl_NA == lcl_NA_check$n_tot){
        ...
        ## so to qualify as trace behavior the duration of the burst must be
        ...
        ## Classify non-straight bursts (bounce, back-and-forth)
      } else{
